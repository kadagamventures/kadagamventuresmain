require("dotenv").config({
  ...
  async function migrate() {
    try {
      ...
      const services = await VideoProduction.find();
  
      console.log(`Found ${services.length} Video Production records`);
  
      for (const service of services) {
  
        const exists = await prisma.videoProduction.findUnique({
          where: {
            slug: service.slug,
          },
        });
        ...
        for (const plan of service.pricingPlans) {
          ...
          await prisma.videoProductionPricingPlan.create({
            data: {
              videoProductionId: createdService.id,
      ...
      console.log("✅ Video Production migration completed.");
