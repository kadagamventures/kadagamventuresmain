require("dotenv").config({
  ...
  async function migrate() {
    try {
      ...
      const services = await SoftwareTestingQA.find();
  
      console.log(`Found ${services.length} Software Testing & QA records`);
  
      for (const service of services) {
  
        const exists = await prisma.softwareTestingQA.findUnique({
          where: {
            slug: service.slug,
          },
        });
        ...
        const createdService =
          await prisma.softwareTestingQA.create({
            data: {
              title: service.title,
              slug: service.slug,
        ...
        for (const plan of service.pricingPlans) {
          ...
          await prisma.softwareTestingQAPricingPlan.create({
            data: {
              softwareTestingQAId: createdService.id,
      ...
      console.log("✅ Software Testing & QA migration completed.");
